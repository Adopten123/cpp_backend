    ...
    ResponseData ProcessRequest(std::string_view target, /*unsigned http_version, std::string_view method, */Send&& send, const http::request<Body, http::basic_fields<Allocator>>&& req/*const json::object& body, const std::string_view bearer*/) {
        ...
        if (splitted[2] == RestApiLiterals::GAME) {
            ...
            if (splitted[3] == RestApiLiterals::PLAYER) {
                if (splitted.size() == 4) {
                    Sender::SendAPIResponse(http::status::bad_request, HttpBodies::BAD_REQUEST, http_version, std::move(send));
                    return { http::status::bad_request, ContentType::APP_JSON };
                }
                if (splitted[4] == RestApiLiterals::ACTION) {
                    ...
                    if (content_type != ContentType::APP_JSON) {
                        Sender::SendAPIResponse(http::status::bad_request, HttpBodies::INVALID_CONTENT_TYPE, req.version(), std::move(send));
                        return { http::status::bad_request, ContentType::APP_JSON };
                    }
                    json::object body;
                    try {
                    ...
                    double speed = player->GetSession()->GetSpeed();
                    if (move_value == "U") {
                        player->GetDog()->Move(model::Direction::NORTH, speed);
                        Sender::SendAPIResponse(http::status::ok, "{}"sv, http_version, std::move(send));
                        return { http::status::ok, ContentType::APP_JSON };
                    }
                    if (move_value == "D") {
                        player->GetDog()->Move(model::Direction::SOUTH, speed);
                        Sender::SendAPIResponse(http::status::ok, "{}"sv, http_version, std::move(send));
                        return { http::status::ok, ContentType::APP_JSON };
                    }
                    if (move_value == "L") {
                        player->GetDog()->Move(model::Direction::WEST, speed);
                        Sender::SendAPIResponse(http::status::ok, "{}"sv, http_version, std::move(send));
                        return { http::status::ok, ContentType::APP_JSON };
                    }
                    if (move_value == "R") {
                        player->GetDog()->Move(model::Direction::EAST, speed);
                        Sender::SendAPIResponse(http::status::ok, "{}"sv, http_version, std::move(send));
                        return { http::status::ok, ContentType::APP_JSON };
                    }
                    if (move_value == "") {
...
private:
    ...
    //net::io_context& ioc_;
    //ìá õðàíèòü ìàïó ñåññèé ê ñòðåíäàì
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send, std::function<void(ResponseData&&)> handle) {
        ...
        case RequestType::API:
        {
            net::dispatch(api_handler_->GetStrand(), [self = shared_from_this(), string_target_ = std::move(string_target)
                                                     ...
                                                     , handle_ = std::move(handle)/*, body_ = std::move(body)*/]() {
