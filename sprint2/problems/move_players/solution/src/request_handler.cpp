...
RequestHandler::RequestHandler(model::Game& game, const char* path_to_static, net::io_context& ioc)
    : game_{ game },
    root_path_(path_to_static),
    api_handler_(std::make_shared<APIRequestHandler>(game, ioc)){
}

APIRequestHandler::APIRequestHandler(model::Game& game, net::io_context& ioc)
    : game_{ game },
